...
def parse_arguments():
    ...
    parser.add_argument("--confidence_type", default="Common", type=str, choices=["Common", "LLM", "And", "Or"])
    ...
    parser.add_argument("--score_type", default="noisy-or", type=str, choices=["noisy-or", "sum", "mean", "min", "max"])
        ...
        if test_query[1] in rules_dict:
            ...
            if cands_dict[0]:
                for s in range(len(args)):
                    ...
                    noisy_or_cands = dict(sorted(cands_scores.items(), key=lambda x: x[1], reverse=True))
...
def apply_rules_in_parallel(parsed, test_data, windown_subgraph, rules_dict, args, score_func):
    ...
    output = Parallel(n_jobs=parsed["num_processes"])(
        delayed(apply_rules)(
            i, num_queries, parsed, test_data, windown_subgraph, rules_dict, args, score_func, parsed["top_k"]
        )
...
def main():
    ...
    new_rules_dict, sort_rules_dict = ra.filter_rules(
        ...
        confidence_type=parsed["confidence_type"],
    )
    ...
    final_all_candidates, final_all_timestamp, final_no_cands_counter = apply_rules_in_parallel(
        parsed, test_data, windown_subgraph, rules_dict, args, score_func
    )
