...
def get_params():
    ...
    parser.add_argument(
        ...
        choices=[
            "transformer",
            "timestamp",
            "based_source_with_timestamp",
            "origin",
            "fusion",
            "fusion_with_weight",
            "fusion_with_source",
            "fusion_with_relation",
            "TADistmult",
            "TADistmult_with_recent",
            "frequcy_only",
            "new_origin_frequency",
            "TiRGN",
            "REGCN",
        ],
        default="timestamp",
    ...
    parser.add_argument(
        "--multi-step", action="store_true", default=False, help="do multi-steps inference without ground truth"
    )
    parser.add_argument(
        "--topk", type=int, default=50, help="choose top k entities as results when do multi-steps without ground truth"
    )
    ...
    parser.add_argument(
        "--skip-connect", action="store_true", default=False, help="whether to use skip connect in a RGCN Unit"
    )
    ...
    parser.add_argument(
        "--self-loop", action="store_true", default=True, help="perform layer normalization in every layer of gcn "
    )
    parser.add_argument(
        "--layer-norm", action="store_true", default=False, help="perform layer normalization in every layer of gcn "
    )
    parser.add_argument(
        "--relation-prediction", action="store_true", default=False, help="add relation prediction loss"
    )
    ...
    parser.add_argument(
        "--grid-search", action="store_true", default=False, help="perform grid search for best configuration"
    )
    parser.add_argument(
        "-tune",
        "--tune",
